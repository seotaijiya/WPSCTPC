...
def test_NP(channel, log_channel, model, sigma, tx_max, eta, Rth, Eth):
    ...
    print("")
    print("CDF")
    sorted_rate_CDF = np.sort(H_rate)
    sorted_CDF_mat = []
    for iii in range(31):
        cur_val = np.sum(sorted_rate_CDF <= iii)/sorted_rate_CDF.shape[0]
        sorted_CDF_mat.append(cur_val)
    print(np.array(sorted_CDF_mat))
    print("")
...
def test_NP_2(channel, log_channel, model, sigma, tx_max, eta, Rth, Eth):
    Num_sample = channel.shape[0]
    Num_user = channel.shape[1]
    H_rate = []
    E_amount = []
    ...
    s_time = time.time()
    tx_power = model.predict(log_channel)
    tx_power = np.ones(tx_power.shape)
    e_time = time.time()
...
def test_NP_3(channel, log_channel, model, sigma, tx_max, eta, Rth, Eth):
    Num_sample = channel.shape[0]
    Num_user = channel.shape[1]
    H_rate = []
    E_amount = []
...
def cal_RATE_EH_H_tf(channel, PL_output, noise, std_ch, harvesting_ratio, error_val):
    ...
    multi_fading_I_err = channel_init[:,0,:,:] * ( (1 - error_val ** 2) ** 0.5 + error_val * tf.random_normal([Num_D2D, Num_D2D+1] ))
    multi_fading_Q_err = channel_init[:,1,:,:] * ( (1 - error_val ** 2) ** 0.5 + error_val * tf.random_normal([Num_D2D, Num_D2D+1] ))


    channel_tot = 0.5 * multi_fading_I_err ** 2 + 0.5 * multi_fading_Q_err ** 2


    channel_H = channel_tot[:, :,:-1]
...
def cal_LOSS_Total_tf(channel, tf_output, noise, tx_max, std_ch, H_rat, R_th_tf, Eth_tf, error_val, lambda_mat):
    ...
    Loss = - tf.constant(1.0) * tf.reduce_mean(tf.nn.relu(H_rate[:,0] - G_rate)) * tf.pow(Loss_2, tf.constant(lambda_mat[0])) * tf.pow(Loss_3,
                                                                                                   tf.constant(lambda_mat[1]))
...
def Total_LOSS_wrapper(input_tensor, noise, tx_max, std_ch, H_rat, R_th_tf, Eth_tf, error_val, lambda_mat):
    def TOTAL_LOSS_tf(y_true, y_pred):
        Loss = cal_LOSS_Total_tf(input_tensor, y_pred, noise, tx_max, std_ch, H_rat, R_th_tf, Eth_tf, error_val,lambda_mat)
        return Loss
...
for i in range(1):
    ...
    for iii in range(1):
        ...
        lambda_mat = np.ones((2, 1)) * 10.0
        #lambda_mat[0] = iii * 4 + 2
        lambda_mat = np.float32(lambda_mat)
        ...
        model_prop.fit(prep_data_tr, LABEL_temp, batch_size=batch_size_set, epochs=num_iter_set, verbose=0)
        ...
        print("")
        print("*" * 50)
        print("Different power")
        print("")
        test_NP_3(data_test_raw, prep_data_val, model_prop, sigma, Pmax, eta, Rth_tf, Eth)
